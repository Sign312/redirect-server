...
function cutQuery(url) {
  ...
  if (urls.length <= 1) {
    ...
    return (
      urls[0] + "?" + urls[1] + "&" + urls.splice(2, urls.length - 1).join("&")
    );
  }
}
...
router.put("*", async function(ctx, next) {
  ...
  if (query.remoteAddress) {
    let remoteAddress = query.remoteAddress;
    delete query.remoteAddress;
    try {
      ctx.body = await redirectPut(
        remoteAddress + cutQuery(ctx.originalUrl),
        body
      );
    } catch (e) {
      ctx.body = e;
    ...
    if (config.remoteAddress) {
      try {
        ctx.body = await redirectPut(
          config.remoteAddress + cutQuery(ctx.originalUrl),
          body
        );
      } catch (e) {
        ctx.body = e;
